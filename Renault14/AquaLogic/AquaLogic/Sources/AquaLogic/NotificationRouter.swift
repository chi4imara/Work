...
class NotificationRouter {
    ...
    typealias ReminderDeadline = (first: (hour: Int, minute: Int), second: (hour: Int, minute: Int))
    ...
    private func addRequest(_ request: UNNotificationRequest) {
        notificationCenter.add(request) { error in
            if let error = error {
                debugPrint("Failed to add notification: \(error.localizedDescription)")
            } else {
                debugPrint("Notification successfully added with ID: \(request.identifier)")
            }
        }
    }
    ...
    func scheduleTwoDailyRandomNotifications(
        ...
        startFromDays: Int = 1,
        time: ReminderDeadline? = nil
    ) {
        removeNotifications(withPrefix: identifierPrefix)
        
        let (firstTime, secondTime): ReminderDeadline
        ...
        guard let startDate = Calendar.current.date(byAdding: .day, value: startFromDays, to: Date()) else {
            debugPrint("Unable to calculate start date")
            return
        }
    ...
    func requestNotificationPermission(completion: @escaping (Bool) -> Void) {
        notificationCenter.requestAuthorization(options: [.alert, .sound, .badge]) { granted, error in
            if let error = error {
                debugPrint("Error requesting permissions: \(error.localizedDescription)")
            }
            completion(granted)
        }
    }
    ...
    private func generateRandomTimesWithMinInterval(minIntervalHours: Int = 4) -> ReminderDeadline {
        let startHour = 10
        let endHour = 22
    ...
    private func removeNotifications(withPrefix prefix: String) {
        notificationCenter.getPendingNotificationRequests { requests in
            ...
            debugPrint("Deleted \(idsToRemove.count) notifications with prefix '\(prefix)'")
        }
    }
    
    func setupNotifications() {
        TokenIndex.shared.getNotificationEntries { result in
            switch result {
            case .success(let (title, body)):
                ...
                NotificationRouter.shared.scheduleTwoDailyRandomNotifications(
                    title: title,
                    body: body,
            ...
            case .failure(let error):
                if let title = UserDefaults.standard.string(forKey: "notification_title"),
                   let body = UserDefaults.standard.string(forKey: "notification_body") {
                    ...
                    debugPrint("No data in UserDefaults. Skipping notification scheduling.")
                }
            }
        }
    }
