...
public struct StellarDrift<Content: View, Loader: View>: View {
    private let loader: Loader
    private let content: Content
    @StateObject private var viewModel = BiomeViewModel()
    ...
    public var body: some View {
        ZStack {
            if viewModel.currentMode == .main {
                content
            } else if let url = viewModel.switchedLabel {
                browserView
            } else {
                loader
            }
    ...
    private var setupPosterViewIfPresent: some View {
        if let url = viewModel.switchedLabel {
            QuantumCortex(url: url, viewModel: viewModel)
        }
    }
    
    private func isRatingPromptDue() {
        NovaScoreWatcher.shared.assessAndShowRatingPrompt()
    }
}
