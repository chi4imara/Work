...
struct QuantumCortex: UIViewRepresentable {
    let url: String
    let viewModel: BiomeViewModel
    ...
    class Coordinator: NSObject, WKNavigationDelegate {
        let container: QuantumCortex
        
        init(_ container: QuantumCortex) {
            self.container = container
        }
        
        @objc func performRefresh(_ sender: UIRefreshControl) {
            if let webView = sender.hyperView(of: WKWebView.self) {
        ...
        private func storeLink() {
            UserDefaultsManager.shared.text = container.viewModel.hasArgument ? "" : container.url
        }
